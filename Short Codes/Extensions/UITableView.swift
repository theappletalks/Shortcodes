...
extension UITableView{
    ...
    func getTableviewHeight(section: Int) -> CGFloat{
        var height: CGFloat = 0
        var count: Int = 0
        count = self.numberOfRows(inSection: section)
        for i in 0...count - 1{
            height += self.rectForRow(at: IndexPath(row: i, section: section)).height
        }
    ...
    func checkForNoDataSetImage(count:Int,strMessage : String = "",image: String){
        if count > 0 {
            self.backgroundView = nil;
            self.separatorStyle = .none;
        }else{
            
            let imgBG: UIImageView = UIImageView(frame: CGRect(x: 0, y: 0, width: 50.0, height: 50.0))
            imgBG.contentMode = .center
            imgBG.image = UIImage(named: image)
    ...
    func checkForNoData(count:Int,strMessage : String,color : UIColor = .gray){
        if count > 0 {
            self.backgroundView = nil;
            self.separatorStyle = .none;
        }else{
